...
def tampilkan_tahap_solusi(nama_siswa, jenis_terdominan):
    ...
    display(HTML("<hr><h2>💡 Tahap 3: Merancang Solusi Berbasis Data & Laporan Akhir</h2>"))
    display(HTML(f"""
    ...
    def on_submit_solusi(sender):
        with output_akhir:
            ...
            display(HTML(f"""
                ...
                <br>
                <h3>🔬 Laporan Temuan dan Apresiasi</h3>
                <div style='border: 1px solid #ced4da; padding: 15px; background-color: #f8f9fa; border-radius: 5px;'>
                    ...
                    <hr>
                    <p style='font-weight: bold;'>🚨 {fakta}</p>
...
def run_analisis_sampah(df, nama_siswa):
    ...
    display(HTML("<h2>📊 Tahap 2: Hasil Analisis Data Audit</h2>"))
...
def tampilan_awal_dan_input():
    ...
    display(HTML("<h1>🗑️ Data Driven Trash Tracker: Misi Mikroplastik Sekolah</h1>"))
    ...
    def on_submit_nama(sender):
        with output_nama:
            ...
            if nama_siswa_input.value:
                display(HTML(f"🎉 Halo, <b>{nama_siswa_input.value}</b>! Siap memulai misi? Lanjutkan ke Tahap 1."))
            else:
...
if __name__ == "__main__":
    ...
    display(HTML("<hr><h2>📥 Tahap 1: Unggah Data Audit Sampah</h2>"))
        ...
        with output_upload:
            ...
            try:
                ...
                if not all(col in df.columns for col in kolom_wajib):
                    display(HTML("⚠️ <span style='color:red;'>Error: Kolom Excel tidak sesuai!</span> Pastikan kolomnya adalah: Tanggal, Jenis Plastik, Berat (gram), Sumber (Kantin/Kelas)."))
                    return

                display(HTML(f"<p style='color:green;'>✅ File <b>{file_name}</b> berhasil diunggah.</p>"))
